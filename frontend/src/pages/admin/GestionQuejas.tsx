...
const GestionQuejas = () => {
    ...
    try {
      ...
      const quejasConEstado = await Promise.all(
        quejas.map(async (queja: Comunicacion) => {
          try {
            const seguimiento = await seguimientoService.getByComunicacionId(queja.id_comunicacion!);
            if (seguimiento) {
  ...
  const descargarPDFComunicacion = async () => {
    ...
    try {
      ...
      // Colores
      const colorAzul: [number, number, number] = [25, 45, 99]; // #192d63
      ...
      // Propuesta de mejora (si existe en las notas)
