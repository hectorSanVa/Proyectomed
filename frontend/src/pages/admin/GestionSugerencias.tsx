...
const GestionSugerencias = () => {
  ...
  }, [searchTerm, filterEstado, filterPrioridad, filterCategoria, fechaDesde, fechaHasta, ordenPor, ordenDireccion, comunicaciones]);
  ...
  const loadData = async () => {
    try {
      ...
      console.log('🔄 Cargando datos de sugerencias...');
      ...
      console.log('✅ Sugerencias procesadas:', sugerenciasConEstado.length);
      setComunicaciones(sugerenciasConEstado);
      setFilteredComunicaciones(sugerenciasConEstado);
      setEstados(estadosData);
      setError(null);
    } catch (error: unknown) {
      console.error('❌ Error al cargar datos:', error);
      let errorMessage = 'Error al cargar las sugerencias. Verifica que el backend esté corriendo en http://localhost:3000';
  ...
  const filterComunicaciones = () => {
    const filtered = comunicaciones.filter((c: ComunicacionConEstado) => {
      ...
      const matchesPrioridad = filterPrioridad === 'Todas' || 
        c.seguimiento?.prioridad === filterPrioridad;
      ...
      return matchesSearch && matchesEstado && matchesCategoria && matchesPrioridad && matchesFecha;
    });
  ...
  const descargarPDFComunicacion = async () => {
    ...
    try {
      ...
      doc.text('FORMATO DE SUGERENCIAS', pageWidth / 2, yPosition, { align: 'center' });
        ...
        <div className="filtros-section">
          <div className="filtros-content">
            ...
            <div className="filtros-selects">
              ...
              <div className="filter-group">
                <label className="filter-label">
                  <MdPriorityHigh className="filter-icon" />
                  Prioridad
                </label>
                <select value={filterPrioridad} onChange={(e) => setFilterPrioridad(e.target.value)} className="filter-select">
                  <option>Todas</option>
                  <option>Baja</option>
                  <option>Media</option>
                  <option>Alta</option>
            ...
            <div className="empty-message">
              <div style={{ fontWeight: 'bold', marginBottom: '0.5rem', fontSize: '1.2rem' }}>
                {comunicaciones.length === 0 
                  ? 'No hay sugerencias registradas en el sistema'
                  : 'No se encontraron sugerencias con los filtros aplicados'}
              </div>
              {comunicaciones.length === 0 && (
                <div style={{ fontSize: '0.9rem', color: '#888', marginTop: '0.5rem' }}>
                  Las sugerencias aparecerán aquí cuando los usuarios las envíen
                </div>
              )}
              </div>
            ) : (
          ...
          <div className="modal-overlay" onClick={() => setShowModal(false)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              <div className="modal-header">
                <h2>
                  {modalMode === 'view' && 'Detalles de la Sugerencia'}
